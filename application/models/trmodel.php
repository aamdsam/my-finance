...
Class Trmodel extends CI_Model
{
	...
	function get_chart()
	{
		...
		$this->db->query(
			'
			DROP TEMPORARY TABLE IF EXISTS trans2
			'
		);
		$this->db->query(
			'
			CREATE TEMPORARY TABLE IF NOT EXISTS trans2
			(
	...
	{
		$this->db->select('a.*,b.* FROM ((SELECT tanggal_transaksi,transaksi,user,keterangan,nomor,id,IF(debet=0,kredit,debet) as nominal FROM transaksi) a)',false);
		$this->db->join('(SELECT IF(debet=0,kredit,debet) as telahbayar,id_piutang FROM transaksi) b','a.nomor=b.id_piutang','left');
		$this->db->where_in('a.transaksi',array(8,14,13));
		$this->db->where('a.user',from_session('user_id'));
		$this->db->where('(nominal-IF(isnull(telahbayar)=1,0,telahbayar))!=0','',false);
